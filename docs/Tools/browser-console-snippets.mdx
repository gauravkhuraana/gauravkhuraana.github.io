---
title: Browser Console Snippets for Testing and Automation
description: Use practical JavaScript browser console snippets to extract YouTube data, inspect web pages, automate repetitive tasks, and support testing workflows.
keywords: [browser console snippets, javascript snippets, browser developer tools, web testing utilities, youtube data extraction, test automation]
image: /img/og/tools-browser-console-snippets-og.png
---
...
## YouTube Channel All Videos Extraction

Extract all video titles and IDs from a YouTube channel's "Videos" page. This snippet auto-scrolls to load all videos, then extracts unique video information with clean titles.

```javascript
// Get all videos from YouTube channel page with auto-scroll
(async () => {
  ...
  anchors.forEach(a => {
    ...
    // Skip junk titles (timestamps, "now playing" labels, etc.)
    if (
      !title ||
    ...
    // Keep the best (longest) title per videoId
    if (!cleaned[videoId] || title.length > cleaned[videoId].title.length) {
      cleaned[videoId] = { title, videoId };
    }
  });

  const results = Object.values(cleaned);

  console.log('Unique videos:', results.length);
...
**Usage Instructions:**
1. Navigate to any YouTube channel's Videos tab (e.g., `youtube.com/@ChannelName/videos`)
2. Open browser developer tools (F12)
3. Go to Console tab
4. Paste and execute the script
5. Wait for auto-scroll to complete (loads all videos)
6. View results in console table format with video titles and IDs

**Key Features:**
- Auto-scrolls to load all videos on the page
- Filters out junk titles (timestamps, "now playing" labels)
- Deduplicates videos and keeps the best title for each
- Returns clean array with `title` and `videoId` for each video
